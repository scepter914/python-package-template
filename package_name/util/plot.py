...
def get_lim_from_multiple_values(
    ...
    for value_list_ in value_lists:
        max_lim, min_lim = get_lim(value_list=value_list_, interval=interval)
...
def get_grid_interval_from_multiple_values(value_lists: List[List[float]]) -> float:
    """Get grid interval value for plot from multiple values list

    Args:
        value_lists (List[List[float]]): Plot values lists
    ...
    for values in value_lists:
        interval_values.append(get_grid_interval(values))
    return max(interval_values)
